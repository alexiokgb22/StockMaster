  ...
  <div class="min-h-screen bg-gray-50 px-6 py-8">
    ...
    <div v-else class="space-y-6">
      ...
      <div class="grid gap-6 md:grid-cols-3">
        <BaseCard>
          <div class="space-y-2">
            ...
            <div class="flex items-baseline gap-2">
              <p class="text-3xl font-bold text-gray-900">{{ warehouse?.usedCapacity }}/{{ warehouse?.totalCapacity }}</p>
            ...
            <div class="space-y-1">
              ...
              <div class="h-2 bg-gray-200 rounded-full overflow-hidden">
                <div class="h-full bg-blue-600 transition-all" :style="{ width: capacityPercent + '%' }" />
              </div>
            </div>
          </div>
        </BaseCard>
        ...
        <BaseCard>
          <div class="space-y-2">
            ...
            <p class="text-lg font-medium text-gray-900">{{ warehouse?.managerName || 'Non assigné' }}</p>
          </div>
        </BaseCard>
        ...
        <BaseCard>
          <div class="space-y-3 text-sm">
            <div>
              ...
              <p class="font-medium text-gray-900">{{ formatDate(warehouse?.createdAt) }}</p>
            </div>
            <div>
              <p class="text-gray-600">Modifié le</p>
              <p class="font-medium text-gray-900">{{ formatDate(warehouse?.updatedAt) }}</p>
            </div>
          </div>
        </BaseCard>
      </div>

      <!-- Onglets Zones / Catégories -->
      <BaseCard>
        ...
        <WarehouseZones
          v-if="activeTab === 'zones' && warehouseId > 0"
      ...
      <BaseCard v-if="canUpdate">
        <h3 class="mb-4 text-lg font-semibold text-gray-900">Modifier les informations</h3>
        <form @submit.prevent="handleUpdate" class="space-y-4">
          <div class="grid gap-4 md:grid-cols-2">
            ...
            <FormField label="Ville">
              <Input v-model="formData.city" :error="formErrors.city" />
            </FormField>
            <FormField label="Adresse" class="md:col-span-2">
              <Input v-model="formData.address" :error="formErrors.address" />
            </FormField>
            <FormField label="Capacité (m³)">
              <Input v-model.number="formData.totalCapacity" type="number" :error="formErrors.totalCapacity" />
            </FormField>
          </div>
          <div class="flex justify-end gap-3 pt-4 border-t">
            <Button variant="secondary" @click="resetForm">Annuler</Button>
            <Button :loading="updating" type="submit">Enregistrer</Button>
          </div>
        </form>
      </BaseCard>
...
const warehouseId = computed(() => warehouse.value?.id ?? 0)
...
const handleUpdate = async () => {
  error.value = null
  if (!validateForm() || !warehouse.value) return
  updating.value = true
  try {
...
const openToggleConfirmation = () => { showToggleConfirmation.value = true }
...
onMounted(async () => {
  ...
  if (!id || isNaN(id)) {
    error.value = 'Identifiant d\'entrepôt invalide'
    loading.value = false
    return
  }
  loading.value = true
